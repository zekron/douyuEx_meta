...
// @downloadURL https://update.greasyfork.org/scripts/394497/DouyuEx-%E6%96%97%E9%B1%BC%E7%9B%B4%E6%92%AD%E9%97%B4%E5%A2%9E%E5%BC%BA%E6%8F%92%E4%BB%B6.user.js
// @updateURL https://update.greasyfork.org/scripts/394497/DouyuEx-%E6%96%97%E9%B1%BC%E7%9B%B4%E6%92%AD%E9%97%B4%E5%A2%9E%E5%BC%BA%E6%8F%92%E4%BB%B6.meta.js
// ==/UserScript==
function init() {
...
function initPkg_FansContinue_Func() {
    document.getElementsByClassName("fans-continue")[0].addEventListener("click", function() {
        ...
        const defValue = null != saved ? saved : "1";
...
function replacePlayerBarrage(text) {
    if (-1 === text.indexOf("player_barrage")) return text;
    return text.replace(/player_barrage\\":0/g, 'player_barrage\\":1').replace(/"player_barrage":0/g, '"player_barrage":1');
}
...
function initPkg_ShowDanmaku() {
    responseHook((url, text) => {
        if (-1 !== url.indexOf("/betard")) return replacePlayerBarrage(text);
        return text;
    });
}
...
function PictureInPictureControl_applyDanmakuStyle(pipWindow) {
    ...
    if (layer) {
        ...
        if (!visible) while (layer.firstChild) layer.removeChild(layer.firstChild);
    }
    if (combo) {
        combo.style.opacity = String(opacity);
        combo.style.display = visible ? "" : "none";
        if (!visible) combo.textContent = "";
    }
}
...
function PictureInPictureControl_stopDocObserver() {
    if (pipDocObserver) {
        if ("function" === typeof pipDocObserver.closeHook) pipDocObserver.closeHook(); else pipDocObserver.disconnect?.();
        pipDocObserver = null;
    }
...
function VideoToolbarMenu_bindEvents() {
    ...
    copyLiveBtn.addEventListener("click", e => {
        e.stopPropagation();
        if ("function" === typeof CopyRealLive_copySelected) CopyRealLive_copySelected();
    });
    audioLineBtn.addEventListener("click", e => {
        e.stopPropagation();
        if ("function" === typeof AudioLine_switch) AudioLine_switch();
    });
    enhancedPipBtn.addEventListener("click", e => {
        e.stopPropagation();
        VideoToolbarMenu_close();
        if ("function" === typeof PictureInPictureControl_launchEnhanced) PictureInPictureControl_launchEnhanced();
    });
...
function applyInlineScriptHooks(node) {
    if ("SCRIPT" !== node.tagName || node.src || !node.textContent) return node;
    const inlineCallbacks = scriptHookCallbackList.filter(item => item.inline);
    if (0 === inlineCallbacks.length) return node;
    let content = node.textContent;
    for (let i = 0; i < inlineCallbacks.length; i++) content = inlineCallbacks[i].callback(content);
...
function handleExternalScript(node, insertFn, targetNode) {
    ...
    for (let k = 0; k < scriptHookCallbackList.length; k++) {
        ...
        if (!item.inline && src.includes(item.url)) callbacks.push(item);
    }
    if (0 === callbacks.length) return false;
...
function initScriptHook() {
    ...
    Node.prototype.appendChild = function(node) {
        if ("SCRIPT" === node.tagName) {
            if (node.src && handleExternalScript(node, originalAppendChild, this)) return document.createDocumentFragment();
    ...
    Node.prototype.insertBefore = function(node, referenceNode) {
        if ("SCRIPT" === node.tagName) {
            if (node.src && handleExternalScript(node, originalInsertBefore, this)) return document.createDocumentFragment();
            node = applyInlineScriptHooks(node);
        }
...
function fetchAndReplace(url, callbacks, targetNode) {
    GM_xmlhttpRequest({
        ...
        onload: function(response) {
            ...
            for (let m = 0; m < callbacks.length; m++) modifiedContent = callbacks[m].callback(modifiedContent);
